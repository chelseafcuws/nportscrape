...
def getFilings(cik):
    ...
    if resp2.status_code == 200: # Checking the response status code. if status code = 200 it means data is available for that CIK.
        ...
        df = df[df['filingDate'] >= yesterday_date] # Filtering the dataFrame for date grater than 2019-01-01
...
try:
    ...
    df = pd.DataFrame(columns=['CIK', 'shortName', 'EndDate', 'PeriodYear', 'PeriodMonth', 'TotalAssets', 'TotalLiabilities', 'NetAssets', 'Subs', 'Reds', 'ReInvest', 'QuarterlySubs', 'QuarterlyReds', 'QuarterlyFlow'])
